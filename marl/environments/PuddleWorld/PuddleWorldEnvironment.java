...
//            visualiser_.setPuddles(puddles_);
    ...
    public void reset(int episodeNo) {
        tuple_.state_ = new PuddleWorldState(DEFAULT);
        if( randomStarts_ ) {
            do {
                tuple_.state_.set(
                        0.95 * Rand.INSTANCE.nextDouble(),
                        0.95 * Rand.INSTANCE.nextDouble());
            } while( isTerminal(tuple_.state_) );
        }
        tuple_.next_ = new PuddleWorldState();
    ...
    public void incrementTime() {
        ...
        double reward = rewardPerStep_ + puddleFine(tuple_.state_);
    ...
    private void informVisualiser(boolean terminal) {
        if( visualiser2_ != null ) {
            ...
            ob.setDouble(0, tuple_.state_.getXPosition()*divisionsX);
            ob.setDouble(1, tuple_.state_.getYPosition()*divisionsY);
