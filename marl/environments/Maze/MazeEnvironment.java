...
public class MazeEnvironment<A extends Agent<MazeEnvironment<A>>>
            ...
            for( int y=0; y<height_; y++ ) {
                for( int x=0; x<width_; x++ ) {
                    ...
                    if( Character.isLowerCase(layout_[y][x]) ) {
                        ...
                        tuple.state_  = new MazeState(tuple.start_);
                        tuple.next_   = new MazeState(tuple.start_);
    ...
    public void reset(int episodeNo)
    {
        ...
        for( int i=0; i<nAgents_; i++ ) {
            tuples_[i].reset();
            tuples_[i].agent_.reset(episodeNo);
        }
        time_ = 0;
    }
    ...
    {
        for( int i=0; i<nAgents_; i++ ) {
            if( tuples_[i].agent_ == null ) {   // assume that a MazeAgent has been provided
                tuples_[i].agent_ = agent;
                tuples_[i].agent_.add(this);
                return true;
            }
        }
    ...
    {
        for( int i=0; i<nAgents_; i++ )
            if( tuples_[i].agent_ == agent )
                return tuples_[i].state_;
    ...
    {
        for( int i=0; i<nAgents_; i++ )
            if( !tuples_[i].state_.equals(tuples_[i].goal_) )
                return false;
    ...
    {
        move(tuple.next_, action);
        if( !isValidPosition(tuple.next_) )
            tuple.next_.set(tuple.state_);  
    }
    ...
    public void incrementTime()
    {
        ...
        // update
        for( int i=0; i<nAgents_; i++ ) {
            tuples_[i].agent_.update(-1.0d, false);
            tuples_[i].addReward(-1.0d);
            if( tuples_[i].state_.equals(tuples_[i].goal_) ) {
                tuples_[i].agent_.update(100.0d, true);
    ...
    private void informVisualiser() {
        if( visualiser_ != null ) {
            ...
            for( int i=0; i<nAgents_; i++ ) {
                ob.setDouble((2*i),   tuples_[i].state_.getXPosition());
                ob.setDouble((2*i)+1, tuples_[i].state_.getYPosition());
            }
            visualiser_.push(ob);
        }
    }
    ...
    private void conflictResolution()
    {
        ...
        for( int i=0; i<nAgents_; i++ ) {
            ...
            if( !tuples_[i].next_.equals(tuples_[i].state_) ) {
                ...
                for( int j=0; j<nAgents_; j++ )
                    if( i != j )    // if not the same agent
                        ...
                        if( tuples_[i].next_.equals(tuples_[j].next_) )
                            hasConflict = true;

                if( !hasConflict )
                    tuples_[i].state_.set(tuples_[i].next_);   // allow the move
                else
                    tuples_[i].next_.set(tuples_[i].state_);   // block movement
            }
        }
    }
