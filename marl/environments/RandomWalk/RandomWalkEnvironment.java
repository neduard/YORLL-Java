    ...
    implements TileCodingEnvironment<RandomWalkState, A>, Model<RandomWalkState>
{
    ...
    public void reset(int episodeNo) {
        tuple_.state_ = new RandomWalkState(DEFAULT_INITIAL_STATE);
        if( randomStarts_ )
            tuple_.state_ = new RandomWalkState(Rand.INSTANCE.nextInt(size_));
        time_             = 0;
        tuple_.next_      = new RandomWalkState();
    ...
    public void incrementTime() {
        ...
        tuple_.agent_.update(rewardPerStep_, false);
    ...
    private void informVisualiser(boolean terminal) {
        if( visualiser_ != null ) {
            ...
            ob.setDouble(0, tuple_.state_.getPosition());
            ob.setDouble(1, 0);
